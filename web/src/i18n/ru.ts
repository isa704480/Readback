...
const catalog = {
  ...
  'sessions.title': 'Сеансы',
  'sessions.intro':
    'Каждый звонок этой организации, новые сверху — включая те, что верно не записали ничего. Откройте любой: что записал конвейер, какие решения были молчаливыми, а какие со вопросом, и аудит того, как. Транскрипта нет.',
  'sessions.loading': 'Загрузка сеансов…',
  'sessions.failed': 'Не удалось загрузить список сеансов. Запись работает через API; проверьте, что он запущен.',
  'sessions.empty': 'Сеансов пока нет. Запустите звонок или демо-фикстуру — и он появится здесь.',
  'sessions.more': 'Более старые сеансы не показаны — это последняя страница.',
  'sessions.loadMore': 'Показать более старые сеансы',
  'sessions.source.live': 'живой',
  'sessions.source.replay': 'фикстура',
  'sessions.source.unknown': 'сеанс',
  'sessions.count.captured': 'записано: {n}',
  'sessions.count.silent': 'молча: {n}',
  'sessions.count.asked': 'со вопросом: {n}',
  'sessions.count.flagged': 'помечено: {n}',
  'sessions.meta.started': 'Начат',
  'sessions.meta.ended': 'Завершён',
  'sessions.meta.ongoing': 'Ещё открыт',
  'sessions.meta.regime': 'Режим уверенности',
  'sessions.section.captures': 'Что записано',
  'sessions.section.questions': 'Заданные вопросы',
  'sessions.section.audit': 'Хронология решений',
  'sessions.rackTitle': 'Что записал конвейер',
  'sessions.noCaptures': 'Этот звонок не записал ничего — верный итог, когда идентификатора, который надо расслышать, не было.',
  'sessions.noTranscript':
    'Транскрипта нет. Запись хранит то, что было записано, и аудит того, как к этому пришли, — и ничего, чем можно было бы восстановить разговор.',
  'sessions.q.position': 'Позиция {n}',
  'sessions.q.answered': 'ответ: {char}',
  'sessions.q.timedout': 'нет ответа',
  'sessions.detail.loading': 'Загрузка сеанса…',
  'sessions.detail.failed': 'Не удалось загрузить этот сеанс.',

  'formats.title': 'Форматы',
  'formats.intro':
    'Форматы идентификаторов, которые Readback может расслышать, и как каждый себя проверяет. В каждом есть контроль, ловящий неверно расслышанный символ до записи — поэтому записанному номеру можно доверять, а сомнение — произнести вслух.',
  'formats.legend.letter': 'буква',
  'formats.legend.digit': 'цифра',
  'formats.legend.check': 'контроль',
  'formats.example': 'Верный пример',
  'formats.method': 'Как работает контроль',
  'formats.advantage': 'Что даёт контроль',
  'formats.sensitive': 'Идентифицирует человека',
  'formats.who': 'Кто читает его вслух',
  'formats.who.iso6346':
    'Линии, экспедиторы, порты и терминалы. Номер называют при букировке, въезде на терминал и по претензиям о повреждении — обычно с площадки, по телефону, на ветру.',
  'formats.who.iban':
    'Банковские и платёжные операции. Диктуют при настройке перевода или исправлении реквизитов, где один неверный символ — это возврат платежа и повторный звонок.',
  'formats.who.vin':
    'Страховщики, дилеры и техпомощь на дороге. Читают с лобового стекла у обочины: семнадцать символов, на улице, один раз.',
  'formats.who.nhs':
    'Клиники, поликлиники и запись пациентов. Номер называет сам пациент, которому может быть больно, а чужая запись — это чужой человек.',
  'formats.who.luhn':
    'Карточные операции. Luhn замечает одну неверную цифру, но не говорит какую, поэтому карту здесь не исправляют: её зачитывают обратно или передают человеку.',
  ...
  'demo.intro':
    'Восемь фикстур в собственном формате сокета — каждая проходит через тот же runner, что и живой звонок: лента, детектор, решатель, принятие решения. В четырёх есть номер контейнера. В четырёх идентификатора нет вовсе, и правильный ответ для них — молчание.',
  'demo.group.captures': 'Четыре с идентификатором',
  'demo.group.refusals': 'Четыре, где нужно промолчать',
  'demo.run': 'Запустить',
  'demo.runAgain': 'Запустить ещё раз',
  'demo.running': 'Фикстура выполняется',
  'demo.failed': 'Сервер не ответил. Демонстрация работает через API; проверьте, что он запущен.',
  'demo.nothing': 'Ничего не записано.',
  'demo.nothingRight': 'Ничего не записано — для этого случая это правильный ответ.',
  'demo.unexpected': 'Что-то было записано. Это ложная запись, и она показана, а не скрыта.',
  'demo.rackTitle': 'Что записал конвейер',
  'demo.fx.clean.title': 'Чистый номер контейнера',
  'demo.fx.clean.body':
    'MSKU 4158005, продиктованный алфавитом НАТО за один ход. Четыре слова меняются в промежуточных результатах, прежде чем устояться.',
  'demo.fx.straddle.title': 'Тот же номер в три хода',
  'demo.fx.straddle.body': 'Заминка в 1,9 секунды внутри кода разрывает его на границах ходов.',
  'demo.fx.visible.title': 'M услышано как N',
  'demo.fx.visible.body':
    'Одна буква подменена в позиции 1. Разные классы вычетов, поэтому контрольная цифра ISO 6346 не сходится.',
  'demo.fx.blind.title': 'K услышано как A — а контрольная цифра всё равно сходится',
  'demo.fx.blind.body':
    'Позиция 3 подменена буквой из того же класса вычетов по модулю 11. Неверный номер арифметически корректен.',
  'demo.fx.conversation.title': '126 секунд разговора, идентификатора нет',
  'demo.fx.conversation.body':
    'Разговор в отделе отгрузки, нарочно засеянный словами, которые и так читаются как символы.',
  'demo.fx.dates.title': 'Две даты на одном дыхании',
  'demo.fx.dates.body':
    'Окно вывоза. «to» читается как цифра 2 и сваривает даты в одну строку из тринадцати цифр.',
  'demo.fx.meter.title': 'Показание счётчика из шестнадцати цифр',
  'demo.fx.meter.body':
    'Продиктовано в отделе коммунальных услуг. Ни несущей фразы, ни идентификатора — шестнадцать цифр, похожих на номер карты.',
  'demo.fx.phone.title': 'Телефонный номер посреди звонка',
  'demo.fx.phone.body': 'Одиннадцатизначный британский мобильный, названный между делом. Ни несущей фразы, ни идентификатора.',
  ...
  'landing.hero.lede':
    'Readback слушает звонок и записывает справочные номера: контейнеры, IBAN, VIN, номера пациентов и карт. Большинство ослышек он исправляет молча. Когда не может — спрашивает про один символ.',
  'landing.hero.scope': 'Слушает и спрашивает по-английски. Язык интерфейса меняет только экран.',
  'landing.hero.demo': 'Запустить демо',
  ...
  'landing.how.step1.title': 'Он ослышивается.',
  'landing.how.step1.body':
    'На плохой линии английские five и nine звучат одинаково. Так же M и N, S и F.',
  'landing.how.step2.title': 'Формат ограничивает ответ.',
  'landing.how.step2.body':
    'В номере контейнера есть контрольный разряд, вычисленный из остальных. У IBAN — mod‑97, у карты — Luhn.',
  'landing.how.step3.title': 'Обычно допустим только один ответ.',
  'landing.how.step3.body':
    'В позицию {position} подходят десять цифр. С контрольным разрядом сходится одна — её и записывают.',
  'landing.scene.label':
    'Номер контейнера в трёх измерениях. В позиции {position} услышано {heard}, а записано {written}: только {written} сходится с контрольным разрядом {check}.',
  'landing.formats.title': 'Какие форматы он читает',
  'landing.formats.label': 'Форматы идентификаторов, по одному на слайд',
  'landing.formats.valid': 'Верный пример',
  'landing.chart.label':
    'Уровень ошибок, который выдерживает система, без ограничения и с ним: ISO 6346 от {isoFrom} до {isoTo}, IBAN от {ibanFrom} до {ibanTo}.',
  'landing.chart.axis': 'Выдерживаемый уровень ошибок',
  'landing.gain.title': 'Ограничение выдерживает в пятнадцать раз больше ошибок.',
  'landing.gain.body':
    'Измерено на восьми акцентах и примерно двенадцати миллионах смоделированных захватов: {iso}× на ISO 6346, {iban}× на IBAN. Знание акцента стоит ±{value}, поэтому его никто не определяет.',
  'landing.gain.ratio.unconstrained': 'без ограничения',
  'landing.gain.ratio.constrained': 'с ограничением',
  'landing.limit.title': 'Чего он не видит.',
  'landing.limit.body':
    'Двенадцать пар букв, среди них B и V, невидимы для контрольного разряда ISO 6346. На них приходится {share}% измеренных ослышек. Там агент спрашивает при любом сомнении, а уверенная ослышка записывается как услышана. Этот остаток измерен, и он не нулевой.',
  'landing.close.title': 'Стойка — это и есть весь интерфейс.',
  'landing.close.body':
    'Расшифровка не хранится. Обратно возвращается номер и число случаев, когда пришлось перебить.',
  // ------------------------------------------------------------- transport --
  ...
  'account.format.iso6346.note.after':
    'Двенадцать из этих пар вдобавок похожи на слух — B/V, K/A, F/P и ещё девять; это 5.3% измеренного веса путаницы. Readback спрашивает про все двенадцать, когда распознаватель помечает символ сомнительным — при посимвольной уверенности так поймано 22 из 23 таких ослышек в бенче на 400 номеров. При блочной уверенности (одно число на целое склеенное слово — обычный случай на живом сокете) он не видит, в каком символе сомневаться, и ослышка, которую арифметика принимает, записывается как услышано: 13 из 400 в том же бенче. Молчание там — не уверенность, а арифметика, которая не видит, и это измерено.',
  ...
  'admin.intro':
    'Всё развёртывание, все организации. Каждое изменение здесь записывается в журнал аудита.',
  'admin.loading': 'Загрузка…',
  'admin.failed': 'Не удалось загрузить: {message}',
  ...
  'admin.tab.quality': 'Качество ИИ',
  'admin.tab.organisations': 'Организации',
  'admin.tab.users': 'Пользователи',
  'admin.tab.live': 'Сейчас',
  'admin.tab.audit': 'Журнал аудита',
  'admin.tab.controls': 'Переключатели',
  'admin.tab.system': 'Система',
  'admin.kpi.organisations': 'Организации',
  'admin.kpi.suspended': 'приостановлено: {n}',
  'admin.kpi.users': 'Пользователи',
  'admin.kpi.usersSub': 'входили за неделю: {active} · отключено: {disabled}',
  'admin.kpi.sessions': 'Сеансы за 24 ч',
  'admin.kpi.sessionsSub': 'за неделю: {week}',
  'admin.kpi.running': 'Идут сейчас',
  'admin.kpi.runningSub': 'из {capacity} слотов',
  'admin.kpi.spend': 'Расход сегодня',
  'admin.kpi.spendSub': '{spent} из {budget} сокет-секунд',
  'admin.kpi.benchmark': 'Бенчмарк',
  'admin.kpi.benchmarkSub': 'верно {correct} из {total} · молча записано неверно: {wrong}',
  'admin.kpi.benchmarkNone': 'Не запускался с момента старта сервера',
  'admin.state.alarm': 'Бюджет на исходе',
  'admin.state.exhausted': 'Бюджет исчерпан',
  'admin.state.livePaused': 'Живой захват приостановлен',
  'admin.state.signupsPaused': 'Регистрация приостановлена',
  'admin.state.normal': 'Все переключатели в норме',
  'admin.q.window': 'Период',
  'admin.q.days': '{n} дн.',
  'admin.q.source': 'Источник',
  'admin.q.source.all': 'Все',
  'admin.q.source.live': 'Живые звонки',
  'admin.q.source.replay': 'Повторы',
  'admin.q.silentCommit': 'Записано без вопроса',
  'admin.q.silentRepair': 'Исправлено молча',
  'admin.q.asked': 'Задан вопрос',
  'admin.q.handover': 'Передано человеку',
  'admin.q.latency': 'Время до записи, медиана',
  'admin.q.latencySub': 'p95 {p95}',
  'admin.q.necessity.title': 'Были ли вопросы нужны?',
  'admin.q.necessity.body':
    'Когда агент спрашивает про один символ, ответ человека — это истина для этой позиции. Если он отличается от услышанного, вопрос предотвратил неверную запись.',
  'admin.q.neededShort': 'Вопрос был нужен',
  'admin.q.needed': 'Нужен',
  'admin.q.unneeded': 'Не нужен',
  'admin.q.judged': 'оценено отвеченных вопросов: {n}',
  'admin.q.none': 'За этот период захватов нет.',
  'admin.q.byFormat': 'По формату',
  'admin.q.byDay': 'По дням',
  'admin.bench.title': 'Бенчмарк против записанной истины',
  'admin.bench.note':
    'Каждая записанная фикстура прогоняется через живой конвейер, а человек отвечает на каждый вопрос верно. Это проверка регрессий на малом корпусе, а не статистическая оценка.',
  'admin.bench.run': 'Запустить бенчмарк',
  'admin.bench.running': 'Выполняется…',
  'admin.bench.summary':
    'верно {correct} из {total} · молча неверно: {wrong} · вопросов в нужной позиции: {right} из {expected}',
  'admin.bench.nothing': 'ничего',
  'admin.result.ok': 'Верно',
  'admin.result.bad': 'Неверно',
  'admin.col.format': 'Формат',
  'admin.col.captures': 'Захваты',
  'admin.col.committed': 'Записано',
  'admin.col.silent': 'Молча',
  'admin.col.asked': 'Спрошено',
  'admin.col.handover': 'Передано',
  'admin.col.day': 'День',
  'admin.col.fixture': 'Фикстура',
  'admin.col.expected': 'Истина',
  'admin.col.written': 'Записано',
  'admin.col.result': 'Итог',
  'admin.col.questions': 'Вопросы',
  'admin.col.name': 'Название',
  'admin.col.status': 'Статус',
  'admin.col.users': 'Польз.',
  'admin.col.sessions7d': 'Сеансы, 7 дн.',
  'admin.col.captures7d': 'Захваты, 7 дн.',
  'admin.col.spendToday': 'Расход сегодня',
  'admin.col.budget': 'Свой бюджет',
  'admin.col.lastActive': 'Последний сеанс',
  'admin.col.actions': 'Действия',
  'admin.col.email': 'Эл. почта',
  'admin.col.org': 'Организация',
  'admin.col.role': 'Роль',
  'admin.col.lastLogin': 'Последний вход',
  'admin.col.session': 'Сеанс',
  'admin.col.source': 'Источник',
  'admin.col.running': 'Идёт',
  'admin.col.audio': 'Микрофон',
  'admin.col.seq': '#',
  'admin.col.at': 'Когда',
  'admin.col.action': 'Действие',
  'admin.col.actor': 'Кто',
  'admin.col.detail': 'Подробности',
  'admin.search': 'Поиск',
  'admin.status.active': 'Активна',
  'admin.status.suspended': 'Приостановлена',
  'admin.status.disabled': 'Отключён',
  'admin.org.searchPlaceholder': 'Название организации',
  'admin.org.suspend': 'Приостановить',
  'admin.org.reinstate': 'Восстановить',
  'admin.org.setBudget': 'Задать бюджет',
  'admin.org.suspendPrompt':
    'Почему {name} приостанавливается? Её идущие сеансы остановятся сейчас же.',
  'admin.org.budgetPrompt':
    'Дневной бюджет для {name} в сокет-секундах. Оставьте пустым, чтобы использовать общий бюджет.',
  'admin.org.budgetDefault': 'общий',
  'admin.org.budgetInvalid': 'Введите целое число секунд или оставьте поле пустым.',
  'admin.user.searchPlaceholder': 'Почта или имя',
  'admin.user.admin': 'Оператор',
  'admin.user.disable': 'Отключить',
  'admin.user.enable': 'Включить',
  'admin.user.disableConfirm': 'Отключить {email}? Выход произойдёт при следующем запросе.',
  'admin.live.empty': 'Сейчас ничего не идёт.',
  'admin.live.stop': 'Остановить',
  'admin.live.stopConfirm': 'Остановить этот сеанс сейчас?',
  'admin.audit.all': 'Всё',
  'admin.audit.admin': 'Действия операторов',
  'admin.audit.more': 'Показать более старые',
  'admin.audit.empty': 'Событий нет.',
  'admin.ctl.live.title': 'Приостановить живой захват',
  'admin.ctl.live.body':
    'Новые сеансы прогоняют записанные фикстуры вместо открытия сокета AssemblyAI. Идущие сеансы не затрагиваются.',
  'admin.ctl.signups.title': 'Приостановить регистрацию',
  'admin.ctl.signups.body':
    'Новые аккаунты не создаются. Все, у кого аккаунт уже есть, продолжают работать.',
  'admin.sys.environment': 'Окружение',
  'admin.sys.python': 'Python',
  'admin.sys.uptime': 'Время работы',
  'admin.sys.database': 'База данных',
  'admin.sys.key': 'Ключ AssemblyAI',
  'admin.sys.present': 'есть',
  'admin.sys.absent': 'нет',
  'admin.sys.live': 'Живой захват',
  'admin.sys.replayMode': 'Режим повтора',
  'admin.sys.consent': 'Версия согласия',
  'admin.sys.cap': 'Лимит сеанса',
  'admin.sys.slots': 'Одновременных сеансов',
  'admin.sys.budget': 'Дневной бюджет',
  'admin.sys.proxy': 'Доверенных прокси',
  'admin.sys.docs': 'Документация API',
  'admin.sys.cors': 'Разрешённые источники',
