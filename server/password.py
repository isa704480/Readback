...
Ported from salvage/password.py with the user-agent changed and one
predictability rule narrowed -- see the DIVERGES note in _predictable(). The
three-way split in the note below is the reason the client can show a
strength verdict with the server switched off: points 1 and 2 are
arithmetic and are mirrored in web/src/lib/password.ts, and only point 3
needs this process. web/tests/password-parity is what keeps the two copies
from drifting -- if they disagree, the meter says Strong and the signup
says no, which is worse than having no meter at all.
...
def _predictable(password: str) -> list[str]:
    ...
    #
    # DIVERGES FROM salvage/password.py, and it is a fix rather than a port
    # error. The salvaged rule was `\D*\d{1,4}[sym]?`, and `\D*` swallows
    # separators as happily as letters, so it also rejected "Tarmoq-Qishloq-42!"
    # -- eighteen characters, four classes, two unrelated words, a genuinely
    # strong passphrase whose only crime is ending in a number. A policy that
    # refuses good passwords teaches people to fight the meter, which is the
    # opposite of what it is for.
    #
    # Restricting the head to a single alphabetic word keeps every case the rule
    # was written for and drops the false positives. Measured over the vectors in
    # tests/test_auth.py: still catches Summer2024!, Password1!, Welcome2024;
    # no longer catches Tarmoq-Qishloq-42! or correct-horse-battery-9.
    if re.fullmatch(r"[A-Za-z]+\d{1,4}[!@#$%^&*]?", password or "x"):
